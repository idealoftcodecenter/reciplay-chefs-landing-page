...
if (isset($_POST['submit'])) {
    ...
    if (nameLike('subject', 4, 30, 'Please select Subject', 'Minimum 4 characters', 'Maximum 128 characters', 'Please select a valid subject')) {
        $subject = filter_var($_POST['subject'], FILTER_SANITIZE_ADD_SLASHES);
    }
    ...
    if (nameLike('entity_name', 4, 64, 'Please provide Your entity name', 'Minimum 4 characters', 'Maximum 30 characters', 'Please enter valid entity name')) {
        $entity_name = filter_var($_POST['entity_name'], FILTER_SANITIZE_ADD_SLASHES);
    }

    if (!isset($_POST['mobile_email']) || empty(trim($_POST['mobile_email'])) || strlen($_POST['mobile_email']) < 8 || strlen($_POST['mobile_email']) > 20) {
        $errors['mobile_email'] = 'Please enter a valid contact number or E-mail ID';
    } else {
        $mobile_email = filter_var($_POST['mobile_email'], FILTER_SANITIZE_ADD_SLASHES);
    }
    ...
    if (count($errors) == 0) {
        ...
        $body = '<table border="1" cellpadding="10" cellspacing="0" style="border: 1px solid #eee;">
						<tr><th bgcolor="#ECE5D8" colspan="2"><h4>New Query</h4></th></tr>
